...
class TestTable : public TestBase {
 public:
  void MakeExample1(int length, bool addColDate = false) {
    auto f0 = field("f0", int32());
    auto f1 = field("f1", uint8());
    auto f2 = field("f2", int16());
    auto f3 = field("f3", utf8());
    auto f4 = field("f4", date64());
    ...
    {
      std::string tmp = std::string("toto") + std::to_string(i);
      
      sb.Append(tmp.c_str(), tmp.length());
    } 
    ...
    arrays_ = {MakeRandomArray<Int32Array>(length), MakeRandomArray<UInt8Array>(length),
               MakeRandomArray<Int16Array>(length), str_array};

    if(addColDate)
    {
        arrays_.push_back(dt_array);
    } 
    columns_ = {std::make_shared<Column>(schema_->field(0), arrays_[0]),
                ...
                std::make_shared<Column>(schema_->field(2), arrays_[2]),
		            std::make_shared<Column>(schema_->field(3), arrays_[3])};

    if(addColDate)
    {
        columns_.push_back(std::make_shared<Column>(schema_->field(4), arrays_[4]));
    } 
  }
 ...
 protected:
  ...
  int64_t now_ = std::chrono::system_clock::now().time_since_epoch() / 
...
TEST_F(TestTable, Ctors) {
  ...
  ASSERT_EQ(4, table_->num_columns());
...
TEST_F(TestTable, Values) {
  const int length = 100;
  MakeExample1(length, true);

  table_ = Table::Make(schema_, columns_);
  ASSERT_OK(table_->Validate());
  ASSERT_EQ(length, table_->num_rows());
  ASSERT_EQ(5, table_->num_columns());
  
  auto dta = table_->column(3)->data();
  ASSERT_EQ(dta->num_chunks(),1);
  auto chk0 = dta->chunk(0);
  ASSERT_EQ(chk0->length(), length);
  
  const auto data = chk0->data();
  ASSERT_EQ(data->length, length);
  auto stringArray = std::static_pointer_cast<arrow::StringArray>(chk0);
  
  ASSERT_EQ(stringArray->GetString(0), std::string("toto0"));
  ASSERT_EQ(stringArray->GetString(1), std::string("toto1"));
  

  auto dta4 = table_->column(4)->data();
  ASSERT_EQ(dta4->num_chunks(),1);
  auto chk4 = dta4->chunk(0);
  ASSERT_EQ(chk4->length(), length);
  
  const auto data4 = chk4->data();
  ASSERT_EQ(data4->length, length);
  auto date64Array = std::static_pointer_cast<arrow::Date64Array>(chk4);
  
  ASSERT_EQ(date64Array->Value(0) , now_);



  /*
auto cost_components = std::static_pointer_cast<arrow::ListArray(
    table_->column(3)->data()->chunk(0));
...
TEST_F(TestTable, RemoveColumn) {
  ...
  auto ex_schema = ::arrow::schema({schema_->field(1), schema_->field(2), schema_->field(3)});
  std::vector<std::shared_ptr<Column>> ex_columns = {table.column(1), table.column(2), table.column(3)};

  auto expected = Table::Make(ex_schema, ex_columns);
  ASSERT_TRUE(result->Equals(*expected));

  ASSERT_OK(table.RemoveColumn(1, &result));
  ex_schema = ::arrow::schema({schema_->field(0), schema_->field(2), schema_->field(3)});
  ex_columns = {table.column(0), table.column(2), table.column(3)};

  expected = Table::Make(ex_schema, ex_columns);
  ASSERT_TRUE(result->Equals(*expected));

  ASSERT_OK(table.RemoveColumn(3, &result));
  ex_schema = ::arrow::schema({schema_->field(0), schema_->field(1), schema_->field(2)});
...
TEST_F(TestTable, SetColumn) {
  ...
  auto ex_schema =
      ::arrow::schema({schema_->field(1), schema_->field(1), schema_->field(2), schema_->field(3)});
...
TEST_F(TestTable, AddColumn) {
  ...
  status = table.AddColumn(5, columns_[0], &result);
  ASSERT_TRUE(status.IsInvalid());
  ...
  auto ex_schema = ::arrow::schema(
      {schema_->field(0), schema_->field(0), schema_->field(1), schema_->field(2), schema_->field(3)});

  auto expected = Table::Make(
      ex_schema, {table.column(0), table.column(0), table.column(1), table.column(2), table.column(3)});
  ASSERT_TRUE(result->Equals(*expected));
  ...
  ex_schema = ::arrow::schema(
      {schema_->field(0), schema_->field(1), schema_->field(0), schema_->field(2), schema_->field(3)});
  expected = Table::Make(
      ex_schema, {table.column(0), table.column(1), table.column(0), table.column(2), table.column(3)});
  ASSERT_TRUE(result->Equals(*expected));

  ASSERT_OK(table.AddColumn(4, columns_[0], &result));
  ex_schema = ::arrow::schema(
      {schema_->field(0), schema_->field(1), schema_->field(2),schema_->field(3), schema_->field(0)});
  expected = Table::Make(
      ex_schema, {table.column(0), table.column(1), table.column(2), table.column(3), table.column(0)});
  ASSERT_TRUE(result->Equals(*expected));
}
